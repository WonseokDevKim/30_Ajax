	...
	window.onload = function(e) {
		...
		regForm.onsubmit = function() {
			...
			if(memberId != '' && passwd != '' && memberName != '' && nickname != '' && email != '') {
				...
				// 비동기시 코드는 실행되고 httpRequest객체는 따로 서버의 응답을 기다림
				// readyState 상태가 변경되면 
			    httpRequest.onreadystatechange = () => {
			    	// readyState DONE (서버에서 응답이 왔으면)
					...
					// Post 방식의 비동기 전송 (3번째 파라미터가 동기/비동기)
			    httpRequest.open('POST', ctx+'/confirmId.do', true);
					// Post 방식의 동기 전송
//			  httpRequest.open('POST', ctx+'/confirmId.do', false);
